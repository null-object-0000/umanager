  ...
  return <div className="local-deb-layer">
    <section className="local-deb-dialog removal-dialog" role="dialog" aria-modal="true" aria-label={`卸载 ${item.displayName}`}>
      ...
      <div className="local-deb-content">
        ...
        {dryRun && !removed && <><div className="dry-run-success"><strong>✓ 卸载前复核通过</strong><span>helper 已重新核对白名单、当前版本、架构和不可变计划；本次未修改系统。</span></div><button className="remove-confirm-button" disabled={busy !== null} onClick={() => void run("remove", () => { setProgressEvents([]); return removeManagedPackage(plan!.plan.planId, (event) => appendProgress(setProgressEvents, event)); }, (value) => { setRemoved(value); onRemoved(); })}>{busy === "remove" ? "正在卸载…" : `再次授权并卸载 ${item.displayName}`}</button></>}
  ...
  return <div className="local-deb-layer">
    <section className="local-deb-dialog removal-dialog self-removal-dialog" role="dialog" aria-modal="true" aria-label="卸载 UManager">
      ...
      <div className="local-deb-content">
        ...
        {dryRun && !removed && <><div className="dry-run-success"><strong>✓ 自卸载前复核通过</strong><span>helper 已重新核对动作、固定包名、版本、架构和不可变计划；本次未修改系统。</span></div><button className="remove-confirm-button" disabled={busy !== null} onClick={() => void run("remove", () => { setProgressEvents([]); return removeUmanager(plan!.plan.planId, (event) => appendProgress(setProgressEvents, event)); }, setRemoved)}>{busy === "remove" ? "正在卸载 UManager…" : "再次授权并卸载 UManager"}</button></>}
        <OperationLogPanel events={progressEvents} running={busy === "remove"}/>
  ...
  return <div className="drawer-layer" onMouseDown={(event) => { if (event.currentTarget === event.target && busy === null) onClose(); }}><aside className="detail-drawer" aria-label={`${item.displayName} 详情`}>
    ...
    {details && <div className="drawer-content">
      ...
      {downloadResult?.verified && <section className="detail-section final-plan-section"><h3>最终更新计划</h3>
        ...
        {dryRun && !installed && <><div className="dry-run-success"><strong>✓ 安装前复核通过</strong><span>helper 已重新核对来源、版本、架构和缓存中的安装包，本次未修改系统。</span></div><button className="install-local-button" disabled={busy !== null} onClick={() => void run("install", () => { setProgressEvents([]); return installPackage(operationPlan!.plan.planId, (event) => appendProgress(setProgressEvents, event)); }, (value) => { setInstalled(value); onInstalled(); })}>{busy === "install" ? "正在更新…" : `再次授权并更新 ${item.displayName}`}</button></>}
        <OperationLogPanel events={progressEvents} running={busy === "install"}/>
