...
- **内置「中国节假日日历」扩展**（`umanager-calendar@umanager.app`）：扩展文件（`metadata.json` / `extension.js` / `stylesheet.css` / `holidays.json`）随 App 编译打包（`include_str!`），离线可用。扩展 hook GNOME 顶部日历（`Main.panel.statusArea.dateMenu._calendar` 的 `_rebuildCalendar` 与菜单 `open-state-changed`），对 `calendar-day` 按钮追加样式类（红=法定休、绿=调休上班）并把节日标记直接写进日期 label（GNOME 45+ 已移除 `St.Widget.set_tooltip_text()`，故不用悬浮提示）。数据格式为 `{"days": [{"date": "YYYY-MM-DD", "name": "…", "isOffDay": bool}]}`。标注规则：调休上班日只标「班」；节假日「首日」（前一日的节日名不同才视为新首日）显示完整节日名（如「国庆节」「春节」「除夕」，除夕与春节相邻时各自显示本名）；段内其余天只标「休」。
- **节假日数据**：官方数据源自 [NateScarlet/holiday-cn](https://github.com/NateScarlet/holiday-cn)（2024–2026 已公布，含调休/连休）。**固定节日补档**（`src-tauri/src/holiday_fixed.rs`）：国务院公布的"放几天假/哪天调休"只覆盖当年；而**节假日本身的日期**由法律/历法固定（元旦 1/1、劳动节 5/1、国庆 10/1、清明约 4/5、以及经农历换算的春节/除夕/端午/中秋），因此在官方未公布年份（2027+）仍按"节日当天必休"补档，**只标当天、绝不猜调休/连休**。农历换算用内置 1900–2100 数据表（经典算法，已用 2024–2026 官方日期回测校准），清明用太阳黄经近似公式。「更新节假日数据」通过 `restricted_client`（仅允许 `raw.githubusercontent.com`、HTTPS-only、大小上限 256 KiB）拉取各年 JSON，已公布年份原样采用、未公布年份补固定节日当天，合并写回扩展目录 `holidays.json`。这是唯一的外网数据源，且只写用户扩展目录，不触碰任何系统路径。
- **生效时机 / 热重载**：GNOME Shell 只在登录时扫描扩展目录（运行中的 Shell 不识别新目录），因此 Wayland 会话下**新安装**的扩展无法即时 `enable`。安装时除写入扩展文件外，还会把 uuid 写入持久化启用列表（`gsettings org.gnome.shell enabled-extensions`，重登后 Shell 自动启用），`gnome-extensions enable` 仅作即时尝试、失败不报错；UI 会显示「待重登生效」并提示注销重登（或 Alt+F2 `r` 重启 Shell）。对**已安装**的扩展，`extension.js` 在日历菜单每次打开时（`open-state-changed`）比对代码/样式文件（js/css/metadata 的 mtime，排除数据文件 holidays.json 以免循环），若发现更新则调用 `Main.extensionManager.reloadExtension(uuid)` 无注销热重载；数据文件变化则仅内存重读。**注意：不要监视自己的代码文件做持续监听，那会 reload 死循环，要改成"打开日历瞬间比对 mtime、更新则 reload 一次"。**
